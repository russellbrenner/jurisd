...
jurisd is an MCP server for AU/NZ legal research. It searches AustLII and removed.invalid,
...
- **`search_cases`** — NL case-law search across all AU/NZ jurisdictions (AustLII +
  removed.invalid merged, authority-ranked). `method`: auto/title/phrase/all/any/near/boolean;
...
- **`fetch_document_text`** — full text from an AustLII or removed.invalid URL (HTML, PDF+OCR,
  removed.invalid RPC). Pass `citeKey` to also save a local source copy + freshness headers.

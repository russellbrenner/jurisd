...
function skipEscStringSequence(value, index) {
    let cursor = index + 2;
    while (cursor < value.length) {
        const code = value.charCodeAt(cursor);
        if (code === 0x07)
...
function renderSearchOutput(contract, parsed) {
    ...
    if (resultsValue.length === 0) {
        ...
        resultsValue.forEach((item, index) => {
            if (isRecord(item))
                lines.push(...renderSearchResult(item, index));
            else
...
function renderListDataModules(parsed) {
    ...
    if (parsed.modules.length === 0) {
        lines.push("No installed ready modules found. Local recall commands will return not-found results.");
        return lines.join("\n");
    }
...
function renderProvision(parsed) {
    if (!isRecord(parsed) || !("found" in parsed))
        return undefined;
...
function renderActStructureNode(node, depth = 0, lines = []) {
    const indent = "  ".repeat(depth);
    lines.push(`${indent}- ${asString(node.label) ?? "(unlabelled node)"}`);
    const children = Array.isArray(node.children) ? node.children : [];
    for (const child of children) {
        if (isRecord(child))
            renderActStructureNode(child, depth + 1, lines);
    }
    return lines;
}
function renderActStructure(parsed) {
    if (!isRecord(parsed) || !("found" in parsed))
        return undefined;
    if (parsed.found !== true) {
    ...
    if (isRecord(parsed.root))
        lines.push(...renderActStructureNode(parsed.root));
    return lines.join("\n");
}
function renderFindCiting(parsed) {
...
function renderSemanticSearch(parsed) {
    if (!isRecord(parsed) || !Array.isArray(parsed.hits))
        return undefined;
    ...
    parsed.hits.forEach((hit, index) => {
        ...
        lines.push(`   ${[provision, score !== undefined ? `score=${score.toFixed(4)}` : undefined].filter(Boolean).join(", ")}`);
...
export function renderTuiToolResult(contract, result) {
    const parsed = parseJsonText(result.text);
    const rendered = contract.id === "search.cases" || contract.id === "search.legislation"
...
function isTuiExecutableContract(contract) {
    ...
    if (TUI_LOCAL_READ_COMMAND_IDS.has(contract.id)) {
        return (contract.sideEffectClass === "read_only_query" ||
            contract.sideEffectClass === "local_metadata_read");
    }
    return (TUI_ACCEPTED_WEB_READ_COMMAND_IDS.has(contract.id) &&
...
export function renderTuiHelp(token) {
    if (!token) {
        return [
    ...
    const contract = findTuiCommandCandidate(token);
    if (!contract)
        return `unknown help topic: ${token}`;

...
function normaliseExaSearchType(raw) {
...
function matchesRequestedJurisdiction(url, options) {
 ...
 */
export async function searchAustliiViaExaWithStatus(query, options, limit) {
    ...
    for (const r of json.results ?? []) {
        ...
        if (wantLegis ? !isLegis : !isCase)
            continue;
        if (!matchesRequestedJurisdiction(url, options))
            continue;
        if (seen.has(url))
            continue;
        seen.add(url);

...
export function calculateAuthorityScore(result) {
    let score = 0;
 ...
 */
export function extractReportedCitation(text) {
 ...
 */
export function isCaseNameQuery(query) {
 ...
 */
export function determineSortMode(query, options) {
 ...
 * ```
 */
export async function searchAustLii(query, options) {
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new AustLiiError(`AustLII search failed: ${message}`, undefined, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export function boostTitleMatches(results, query) {
    ...
    const scored = results.map((result) => {
        ...
        const matchingWords = titleWords.filter((word) => word.length > 2 && queryWords.has(word)).length;
...
//# sourceMappingURL=austlii.js.map
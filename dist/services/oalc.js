...
async function tryLoadDuckDB() {
    ...
    try {
        ...
        if (err instanceof Error &&
            ...
            console.warn("[oalc] @duckdb/node-api is not installed. OALC corpus fallback is disabled. " +
...
async function getConnection() {
    ...
    if (!fs.existsSync(corpusPath)) {
        console.warn(`[oalc] Corpus file not found at ${corpusPath}. ` +
 ...
 *          is unavailable.
 */
export async function lookupByCitation(citation, isLegis) {
    const state = await getConnection();
    if (!state)
        return null;

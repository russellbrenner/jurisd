...
import { IMPLEMENTED_SCHEMA_VERSION, MODULE_NAME_PATTERN, validateManifest, } from "../data/manifest.js";
import { getQueryEmbedder, activeEmbedderDescriptor } from "./embedder.js";
import { baselineAdapter } from "./adapter.js";
const registry = { modules: new Map(), scanned: false };
/**
...
export function setModulesRootForTest(dir, enabled = true) {
    _override = dir === null ? null : { dir, enabled };
    resetRegistry();
}
function modulesDirEffective() {
    return _override?.dir ?? config.modules.dir;
}
function modulesEnabledEffective() {
...
function readManifest(dir) {
    ...
    try {
        ...
        return { error: `manifest.json is not valid JSON: ${err.message}` };
    }
}
/**
 ...
 */
function classifyManifest(manifest, dirName) {
    ...
    if (manifest.name !== dirName) {
        console.warn(`[modules] directory '${dirName}' holds module '${manifest.name}'; manifest identity wins`);
    }
    return { status: "ready" };
}
/**
 ...
 */
function buildEntry(modulesDir, dirName) {
 ...
 */
export function discoverModules(force = false) {
    if (registry.scanned && !force)
        return registry.modules;
    registry.modules.clear();
    registry.scanned = true;
    if (!modulesEnabledEffective())
        return registry.modules;
    ...
    try {
        ...
        console.warn(`[modules] cannot read modules dir ${modulesDir}: ${err.message}`);
        return registry.modules;
    }
    for (const dirName of dirNames) {
        ...
        try {
            ...
            console.warn(`[modules] skipping '${dirName}': ${err.message}`);
            continue;
        }
        if (!entry)
            continue;
...
export function listModules(force = false) {
    return Array.from(discoverModules(force).values());
}
/** Return only `ready` modules. */
export function readyModules(force = false) {
 ...
 */
export async function tryLoadDuckDB() {
    if (_duckdbUnavailable)
        return null;
    try {
        ...
        if (err instanceof Error &&
            ...
            console.warn("[modules] @duckdb/node-api is not installed. Local-module query tools are disabled. " +
 ...
 */
export function viewNames(name) {
    return {
 ...
 */
export async function attachModule(name) {
    ...
    const file = (f) => path.join(entry.dir, f).replace(/'/g, "''");
    const stmts = [
 ...
 */
export async function runModuleQuery(sql, params = []) {
    const db = await getDb();
    if (!db)
        return [];
 ...
 */
export function buildMetadata(entry) {
 ...
 */
export function normaliseProvisionRef(raw) {
...
export function selectModules(opts = {}) {
    ...
    if (opts.jurisdiction) {
        const matching = candidates.filter((m) => m.manifest.coverage.jurisdictions.includes(opts.jurisdiction));
...
export async function getProvision(args) {
    ...
    for (const entry of candidates) {
        ...
        const rows = await runModuleQuery(`SELECT c.chunk_id, c.provision_ref, c.segment_type, c.text,
...
export async function getActStructure(args) {
    ...
    for (const entry of candidates) {
        ...
        const rows = await runModuleQuery(`WITH RECURSIVE tree AS (
       ...
       SELECT node_id, parent_id, label, depth FROM tree ORDER BY depth, label`, [args.act, maxDepth]);
        if (rows.length === 0)
            continue;
 ...
 */
export function listDataModules(opts = {}) {
...
export async function findCiting(args) {
    ...
    for (const entry of candidates) {
        ...
        if (!attached)
            continue;
        const v = viewNames(entry.name);
        const rows = await runModuleQuery(`SELECT DISTINCT src_d.version_id, src_d.citation, src_d.type, src_d.url,
        ...
        LIMIT $3`, [args.target, kinds, limit]);
 ...
 */
function embeddingSpaceMatches(entry, embedder) {
...
function modelIdCompatible(manifestModelId, embedderModelId) {
...
adapter = baselineAdapter) {
    ...
    for (const entry of candidates) {
        if (!embeddingSpaceMatches(entry, embedderDesc)) {
            notes.push(`module '${entry.name}' skipped: embedding descriptor ${JSON.stringify(entry.manifest?.embedding)} does not match the active embedder (${embedderDesc.model_id}, dim ${embedderDesc.dim})`);
            continue;
        }
        const attached = await attachModule(entry.name);
        if (!attached)
    ...
    if (adapter.canRerank && adapter.rerank && topK.length > 0) {
        ...
        for (const c of reordered) {
            ...
            if (hit)
                refined.push({ ...hit, metadata: { ...hit.metadata, enhancement: adapter.label } });
        }
        if (refined.length > 0)
            topK = refined;
    }

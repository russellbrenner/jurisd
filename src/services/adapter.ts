...
export const baselineAdapter: DomainAdapter = {
  canRerank: false,
...
export function readIsaacusConfig(): IsaacusConfig | null {
 ...
 */
export function buildIsaacusAdapter(cfg: IsaacusConfig): DomainAdapter {
  return {
    canRerank: true,
    canExtractiveQA: true,
    label: ISAACUS_LABEL,
    async rerank(query: string, chunks: LocalChunk[]): Promise<LocalChunk[]> {
      try {
        ...
        const body = (await res.json()) as { results?: { index: number; score: number }[] };
        if (!body.results) return chunks;
    ...
    async extractiveQA(
      ...
      try {
        ...
        const body = (await res.json()) as {
          results?: { text: string; start: number; end: number }[];
        };
...
async function isReachable(cfg: IsaacusConfig): Promise<boolean> {
  try {
 ...
 */
export async function probeDomainAdapter(
  ...
  if (!reachable) {
    if (!_loggedUnreachable) {
      ...
      console.warn(
        "[adapter] a domain provider is configured but was not reachable at probe time; " +
